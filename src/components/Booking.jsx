      ...
      {isAdmin && (
        <div className="card" style={{ background: '#FEF3C7', border: '1px solid var(--warning)' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center' }}>
            ...
            {selectedSlots.length > 0 && <button onClick={handleAdminClose} className="btn btn-warning btn-sm" style={{ width: 'auto', minHeight: '36px', padding: '4px 16px', fontSize: '13px' }}>Tutup {selectedSlots.length} Slot</button>}
          </div>
          {selectedSlots.length > 0 && <div style={{ marginTop: '8px', fontSize: '13px', color: '#92400E' }}>✅ {selectedSlots.length} slot dipilih</div>}
        </div>
      )}
      ...
      {showReasonModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 999, padding: '16px' }}>
          <div style={{ background: 'white', borderRadius: '12px', padding: '24px', maxWidth: '400px', width: '100%' }}>
            ...
            <p style={{ fontSize: '14px', color: 'var(--gray-500)', marginBottom: '12px' }}>Tambahkan alasan (opsional)</p>
            <input type="text" value={closureReason} onChange={(e) => setClosureReason(e.target.value)} placeholder="Contoh: Maintenance, Private Event, dll" style={{ width: '100%', padding: '10px 12px', border: '2px solid var(--gray-200)', borderRadius: '8px', fontSize: '14px', fontFamily: 'inherit', marginBottom: '16px' }} autoFocus />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {visibleSlots.map((slot) => {
              ...
              return (
                ...
                }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: '8px', backgroundColor: bgColor, border: `2px solid ${borderColor}`, cursor, flexWrap: 'wrap', gap: '8px', transition: 'all 0.2s ease' }}>
                  <div style={{ fontWeight: 600, color: textColor }}>{formatTime(slot.startTime)} - {formatTime(slot.endTime)}</div>
                  <div style={{ flex: 1 }}>
                    {slot.isBooked ? (
